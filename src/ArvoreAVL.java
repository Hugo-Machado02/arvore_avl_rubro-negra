...
public class ArvoreAVL {
    ...
    public No removeNo(No no, int val) {
        ...
        if (val < no.getValorNo()) {
            ...
            if (no.getRepeticao() > 1) {
                // Apenas decrementa a contagem
                no.decrementaRepeticao();
            }
            else{
    ...
    public int realizarBuscar(int valor) {
        return buscar(this.raiz, valor);
    }

    private int buscar(No no, int valor) {
        if (no == null) {
            return 0; // O valor não está presente
        }
